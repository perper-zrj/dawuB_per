...
\textbf{答案： }
圆心 $O$ 处的电场强度大小为 $E = \frac{Q}{\pi^2 \varepsilon_0 R^2}$，方向沿 $y$ 轴负方向。

\textbf{解答过程： }
建立如图所示的直角坐标系。上半圆环（第二象限部分）带正电荷 $+Q$，下半圆环（第三象限部分）带负电荷 $-Q$。它们在圆心 $O$ 处产生的电场可以由电荷元积分或对称性求出。
由于半圆环总长度为 $\pi R$，上下两部分各占四分之一圆弧，其长度均为 $\frac{\pi R}{2}$。线电荷密度大小为：
\begin{equation}\label{eq:7-9-1}
	\lambda = \frac{Q}{\pi R / 2} = \frac{2Q}{\pi R}
\end{equation}
在上半圆环上任取电荷元 $\mathrm{d}q = \lambda R \mathrm{d}\theta$（设极角为 $\theta$，$\theta \in [\frac{\pi}{2}, \pi]$），其在 $O$ 点产生的微场强为：
\begin{align*}
	\mathrm{d}\vec{E}_+ = \frac{1}{4\pi\varepsilon_0} \frac{\mathrm{d}q}{R^2} (-\cos\theta \hat{i} - \sin\theta \hat{j}) = \frac{\lambda \mathrm{d}\theta}{4\pi\varepsilon_0 R} (-\cos\theta \hat{i} - \sin\theta \hat{j})
\end{align*}
积分求得上半圆环在 $O$ 点产生的电场分量：
\begin{align*}
	E_{+x} &= \int_{\pi/2}^{\pi} \frac{\lambda}{4\pi\varepsilon_0 R} (-\cos\theta) \mathrm{d}\theta = \frac{\lambda}{4\pi\varepsilon_0 R} [-\sin\theta]_{\pi/2}^{\pi} = \frac{\lambda}{4\pi\varepsilon_0 R} \\
	E_{+y} &= \int_{\pi/2}^{\pi} \frac{\lambda}{4\pi\varepsilon_0 R} (-\sin\theta) \mathrm{d}\theta = \frac{\lambda}{4\pi\varepsilon_0 R} [\cos\theta]_{\pi/2}^{\pi} = -\frac{\lambda}{4\pi\varepsilon_0 R}
\end{align*}
同理，下半圆环带负电，电荷元 $\mathrm{d}q = -\lambda R \mathrm{d}\theta$（$\theta \in [\pi, \frac{3\pi}{2}]$），其在 $O$ 点产生的电场分量为：
\begin{align*}
	E_{-x} &= \int_{\pi}^{3\pi/2} \frac{-\lambda}{4\pi\varepsilon_0 R} (-\cos\theta) \mathrm{d}\theta = \frac{\lambda}{4\pi\varepsilon_0 R} [\sin\theta]_{\pi}^{3\pi/2} = -\frac{\lambda}{4\pi\varepsilon_0 R} \\
	E_{-y} &= \int_{\pi}^{3\pi/2} \frac{-\lambda}{4\pi\varepsilon_0 R} (-\sin\theta) \mathrm{d}\theta = \frac{\lambda}{4\pi\varepsilon_0 R} [-\cos\theta]_{\pi}^{3\pi/2} = -\frac{\lambda}{4\pi\varepsilon_0 R}
\end{align*}
根据场强叠加原理，圆心 $O$ 处的总电场为：
\begin{equation}\label{eq:7-9-2}
	E_x = E_{+x} + E_{-x} = \frac{\lambda}{4\pi\varepsilon_0 R} - \frac{\lambda}{4\pi\varepsilon_0 R} = 0
\end{equation}
\begin{equation}\label{eq:7-9-3}
	E_y = E_{+y} + E_{-y} = -\frac{\lambda}{4\pi\varepsilon_0 R} - \frac{\lambda}{4\pi\varepsilon_0 R} = -\frac{\lambda}{2\pi\varepsilon_0 R}
\end{equation}
将公式(\ref{eq:7-9-1})代入公式(\ref{eq:7-9-3})，得总场强大小：
\begin{equation}\label{eq:7-9-4}
	E = |E_y| = \frac{1}{2\pi\varepsilon_0 R} \left(\frac{2Q}{\pi R}\right) = \frac{Q}{\pi^2 \varepsilon_0 R^2}
\end{equation}
方向沿 $y$ 轴负向。

\textbf{易混淆知识点： }
1. \textbf{线电荷密度的取值}：电荷 $Q$ 只分布在四分之一圆弧（长度为 $\pi R / 2$）上，而不是整个半圆，因此线密度为 $\lambda = \frac{2Q}{\pi R}$。如果错算成 $\frac{Q}{\pi R}$ 会导致最终结果相差两倍。

2. \textbf{对称性的矢量叠加}：计算总场强时，由对称性可以直接判断出上半环正电荷与下半环负电荷在 $x$ 轴方向上产生的电场大小相等、方向相反，完全抵消；而在 $y$ 轴方向上，两者产生的电场均指向 $y$ 轴负方向，相互叠加。熟练利用对称性可以大幅简化积分步骤。
...
方向垂直于带电平面。若 $\sigma > 0$，场强指向远离平面的方向；若 $\sigma < 0$，场强指向平面的方向。

设平行平面所在位置的法线方向（水平向右）为正向单位矢量 $\vec{n}$。
...
根据电场强度的叠加原理，空间任一点的总场强等于两平面在该点单独产生场强的矢量和：
...
课本例题重现由于期末考机制问题，将只整理一、七、八章。\textcolor{red}{特别强调：七八两章为老师勾画的例题极为重要，七八两张的例题一定要复习！}
...
\begin{enumerate}
	...
	\textbf{易混淆知识点： }
	1. \textbf{高斯面内包围电荷的计算}：在计算柱面外（$r_2 > R$）高斯面内的电荷时，极易错误地将电荷量写为 $\sigma \cdot 2\pi r_2 h$。必须牢记，电荷只存在于真实的带电体（半径为 $R$ 的圆柱面）上，高斯面内部只有半径为 $R$ 的那部分柱面上有电荷，其余空间是真空。
	
	2. \textbf{不同带电体场强的空间衰减规律}：对于无限大均匀带电平面，场强与距离无关（常数）；对于无限长均匀带电直线或圆柱面，场强与距离成反比（$E \propto 1/r$）；对于点电荷或带电球面，场强与距离的平方成反比（$E \propto 1/r^2$）。在检查推导结果时，可以通过量纲和距离的依赖关系来验证正确性。
	
	3. \textbf{圆柱面高斯面底面通量的处理}：必须在解答中明确指出，\textbf{之所以只计算侧面积的通量，是因为上下底面的法向量与电场方向垂直}，导致点积 $\vec{E} \cdot \mathrm{d}\vec{S} = 0$。如果跳过这一步直接写面积，不仅物理过程不完整，在考试中也极其容易丢掉步骤分。
